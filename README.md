...
사용자가 한 문장을 적을 때마다 낡은 한지 위의 금기문이 붉게 드러나는 한국 괴담풍 HTML-in-Canvas 실험입니다. 각 다섯 문장으로 구성된 두 챕터를 진행하며, 첫 챕터를 마치면 위 영상 프레임의 부적이 사라지고 종이가 두 번째 내용으로 다시 채워집니다. 두 번째 챕터까지 마치면 아래 영상 프레임도 열립니다. 실제 입력과 버튼 DOM을 WebGL 텍스처로 올려 종이의 움직임, 먹 번짐, 필름 떨림을 표현하며 웹과 Electron 데스크톱 앱 두 경로를 함께 제공합니다.
